...
class TestFoodLogDataService extends FoodLogDataService {
  ...
  List<CalorieData> processLogsToCalorieData(List<FoodLogHistoryItem> logs, DateTime startDate) {
    ...
    for (var log in logs) {
      ...
      // Add values
      dailyMacros[dayOfWeek]!['protein'] = (dailyMacros[dayOfWeek]!['protein'] ?? 0) + protein;
      dailyMacros[dayOfWeek]!['carbs'] = (dailyMacros[dayOfWeek]!['carbs'] ?? 0) + carbs;
      dailyMacros[dayOfWeek]!['fats'] = (dailyMacros[dayOfWeek]!['fats'] ?? 0) + fat;
  ...
  List<CalorieData> processLogsToWeeklyCalorieData(List<FoodLogHistoryItem> logs, DateTime startDate) {
    ...
    for (var log in logs) {
      ...
      weeklyMacros[weekNumber]!['protein'] = (weeklyMacros[weekNumber]!['protein'] ?? 0) + protein;
      weeklyMacros[weekNumber]!['carbs'] = (weeklyMacros[weekNumber]!['carbs'] ?? 0) + carbs;
      weeklyMacros[weekNumber]!['fats'] = (weeklyMacros[weekNumber]!['fats'] ?? 0) + fat;
  ...
  List<FoodLogHistoryItem> filterLogsForCurrentMonth(List<FoodLogHistoryItem> logs, DateTime firstDayOfMonth) {
  ...
  group('FoodLogDataService', () {
    ...
    group('calculateTotalCalories', () {
      test('calculates total calories correctly', () {
        ...
        // Act
        final result = foodLogDataService.calculateTotalCalories(calorieDataList);
...
FoodLogHistoryItem _createMockFoodLogItem(
    DateTime timestamp, double protein, double carbs, double fat, double calories) {
