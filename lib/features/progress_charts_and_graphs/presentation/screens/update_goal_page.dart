...
class _UpdateGoalPageState extends State<UpdateGoalPage> {
  ...
  void initState() {
    super.initState();
    _goalWeight = double.tryParse(widget.initialGoalWeight.replaceAll('N/A', '60')) ?? 60.0;
  ...
  void _handleDragUpdate(DragUpdateDetails details, double pixelsPerKg) {
    setState(() {
      ...
      if (newWeight >= _minWeight && newWeight <= _maxWeight && newWeight != _goalWeight) {
          ...
          children: [
            ...
            LayoutBuilder(
              builder: (context, constraints) {
                ...
                final double visibleMin = (_goalWeight - _visibleRange/2).clamp(_minWeight, _maxWeight);
                final double visibleMax = (_goalWeight + _visibleRange/2).clamp(_minWeight, _maxWeight);
