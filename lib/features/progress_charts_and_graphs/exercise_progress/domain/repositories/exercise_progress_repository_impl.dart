...
class ExerciseProgressRepositoryImpl implements ExerciseProgressRepository {
  ...
  Future<List<ExerciseData>> getExerciseData(bool isWeeklyView) async {
    ...
    if (isWeeklyView) {
      ...
      final sunday =
          DateTime(monday.year, monday.month, monday.day + 6, 23, 59, 59, 999);
      ...
      for (var log in allLogs) {
        ...
        if ((normalizedLogDate.isAfter(monday) ||
          ...
          dailyData[dayName] =
              (dailyData[dayName] ?? 0) + log.caloriesBurned.toDouble();
        }
      }

      // Convert to list of ExerciseData with proper day order
      final result = dayNames
          .map((dayName) => ExerciseData(dayName, dailyData[dayName] ?? 0))
          .toList();
      ...
      final currentMonthLogs = allLogs
          .where((log) =>
      ...
      for (var log in currentMonthLogs) {
        ...
        weeklyData[weekLabel] =
            (weeklyData[weekLabel] ?? 0) + log.caloriesBurned.toDouble();
      }

      // Create result list in order
      return weekLabels
          .map((week) => ExerciseData(week, weeklyData[week] ?? 0))
          .toList();
    }
  }
  ...
  Future<List<ExerciseType>> getExerciseTypes() async {
    ...
    int cardioPercentage =
        totalCount > 0 ? ((cardioCount / totalCount) * 100).round() : 33;
    int weightliftingPercentage =
        totalCount > 0 ? ((weightliftingCount / totalCount) * 100).round() : 33;
    int smartExercisePercentage =
        totalCount > 0 ? ((smartExerciseCount / totalCount) * 100).round() : 34;

    // Ensure percentages add up to 100%
    final sum =
        cardioPercentage + weightliftingPercentage + smartExercisePercentage;
    if (sum != 100 && totalCount > 0) {
      final diff = 100 - sum;
      // Add the difference to the largest category
      if (cardioCount >= weightliftingCount &&
          cardioCount >= smartExerciseCount) {
        cardioPercentage += diff;
      } else if (weightliftingCount >= cardioCount &&
          weightliftingCount >= smartExerciseCount) {
  ...
  Future<List<PerformanceMetric>> getPerformanceMetrics() async {
    ...
    final consistencyPercentage =
        min(100, (workoutDaysLastWeek.length / 7 * 100).round());
    final previousConsistencyPercentage =
        min(100, (workoutDaysTwoWeeksAgo.length / 7 * 100).round());
    ...
    final Set<String> workoutDates =
        allLogs.map((log) => dateFormat.format(log.timestamp)).toSet();
    ...
    final intensity = workoutsLastWeek > 0
        ? (totalCaloriesLastWeek / workoutsLastWeek / 100).toStringAsFixed(1)
        : '0.0';
  ...
  int _getDayOfYear(DateTime date) {
    ...
    return diffDays +
        1; // +1 because days are 1-indexed (Jan 1 is day 1, not day 0)
  }
  ...
  Future<String> getCompletionPercentage() async {
    ...
    final completionPercentage = min(
        100, (workoutDaysThisWeek.length / targetWorkoutDays * 100).round());
