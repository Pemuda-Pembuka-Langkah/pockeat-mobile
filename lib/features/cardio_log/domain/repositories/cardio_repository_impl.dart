...
class CardioRepositoryImpl implements CardioRepository {
  ...
  Future<String> saveCardioActivity(CardioActivity activity) async {
    try {
      if (activity.userId.isEmpty) {
        throw Exception('Activity must have a user ID');
      }
  ...
  @override
  Future<List<CardioActivity>> getCardioActivitiesByType(
      CardioType type) async {
    try {
      String typeString;
      switch (type) {
  ...
  @override
  Future<List<CardioActivity>> getActivitiesByUser(String userId) async {
    try {
      final querySnapshot = await _firestore
          .collection(_collection)
      ...
      throw Exception('Failed to retrieve user cardio activities: $e');
    }
  }
}
